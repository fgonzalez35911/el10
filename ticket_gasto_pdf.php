...
$stmt = $conexion->prepare("SELECT g.*, u.usuario as operador, u.nombre_completo FROM gastos g LEFT JOIN usuarios u ON g.id_usuario = u.id WHERE g.id = ?");
$stmt->execute([$id_mov]);
...
$pdf->Cell(0, 5, "Cat: " . utf8_decode(strtoupper($mov->categoria)), 0, 1, 'L');
