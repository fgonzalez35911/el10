<?php
// reporte_inflacion.php - REPORTE PDF CON RANGO DE FECHAS
session_start();
...
<head>
    ...
    <title>Reporte_Inflacion_<?php echo $desde; ?>_al_<?php echo $hasta; ?></title>
...
<body>
    ...
    <div id="reporteContenido" class="page">
        ...
        <h3 style="color: #102A57; border-left: 5px solid #102A57; padding-left: 10px; margin-bottom: 20px;">HISTORIAL DE INFLACIÓN APLICADA</h3>

        <table>
            <thead><tr><th>FECHA / HORA</th><th>OPERADOR</th><th>GRUPO AFECTADO</th><th>PRODUCTOS</th><th style="text-align: right;">AUMENTO</th></tr></thead>
            <tbody>
                <?php foreach($registros as $r): ?>
                <tr>
                    <td><?php echo date('d/m/Y H:i', strtotime($r['fecha'])); ?></td>
                    <td><?php echo strtoupper($r['usuario']); ?></td>
                    <td><?php echo strtoupper($r['grupo_afectado']); ?> <br><small style="color:#777; font-size:7pt;"><?php echo $r['accion']=='COSTO'?'Impactó Costo y Venta':'Solo Venta'; ?></small></td>
                    <td><?php echo $r['cantidad_productos']; ?> u.</td>
                    <td style="text-align: right; font-weight: bold; color:#dc3545;">+<?php echo floatval($r['porcentaje']); ?>%</td>
                </tr>
                <?php endforeach; ?>
                <?php if(empty($registros)): ?><tr><td colspan="5" style="text-align:center;">No hubo inflación registrada en este período.</td></tr><?php endif; ?>
            </tbody>
        </table>

        <div class="footer-section">
            <div style="width: 55%; font-size: 8pt; color: #666; line-height: 1.4;">
                <p><strong>DECLARACIÓN:</strong> Documento generado por el sistema de gestión. Refleja las actualizaciones masivas de precios realizadas en el período seleccionado.</p>
            </div>
            <div class="firma-area">
                <?php if(!empty($firmaUsuario)): ?><img src="<?php echo $firmaUsuario; ?>" class="firma-img" alt="Firma"><?php else: ?><div style="height: 70px;"></div><?php endif; ?>
    ...
    function descargarPDF() {
        ...
        const opt = { margin: 0, filename: 'Reporte_Inflacion_<?php echo $desde; ?>_al_<?php echo $hasta; ?>.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
