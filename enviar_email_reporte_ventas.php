<?php
// acciones/enviar_email_reporte_ventas.php - MOTOR DE ENVÍO DE REPORTE DE VENTAS (VANGUARD PRO)
...
try {
    ...
    $mail->addAttachment($_FILES['pdf_file']['tmp_name'], 'Reporte_Ventas_Oficial_' . date('d-m-Y') . '.pdf');
    
    $mail->isHTML(true);
    $mail->Subject = "Reporte de Ventas Oficial - " . $conf->nombre_negocio;
    ...
    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #102A57; border-radius: 10px; overflow: hidden;'>
        <div style='background: #102A57; padding: 20px; text-align: center;'>
            <h2 style='color: white; margin: 0;'>Reporte de Ventas Corporativo</h2>
        </div>
        <div style='padding: 30px; color: #333;'>
            <p>Hola, se adjunta el reporte de ventas oficial generado desde el sistema.</p>
            <p>Este documento es una copia exacta del reporte visualizado en pantalla, conteniendo el detalle de las operaciones y el resumen de recaudación.</p>
            <table width='100%' style='background: #f4f7fa; padding: 15px; border-radius: 5px; margin: 20px 0;'>
                <tr><td><strong>Tipo de Reporte:</strong></td><td align='right'>Ventas y Recaudación</td></tr>
