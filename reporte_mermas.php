<?php
// reporte_mermas.php - REPORTE PDF CORPORATIVO (CON BUSCADOR INTEGRADO)
session_start();
...
try {
    ...
    // 1. Datos de quien genera el reporte (Operador)
    ...
    $buscar = trim($_GET['buscar'] ?? ''); // <--- Capturamos la búsqueda

    $condiciones = ["DATE(m.fecha) >= ?", "DATE(m.fecha) <= ?", "m.motivo NOT LIKE 'Devolución #%'"];
    $parametros = [$desde, $hasta];
    if($f_usu !== '') { $condiciones[] = "m.id_usuario = ?"; $parametros[] = $f_usu; }
    
    // <--- Aplicamos el filtro de búsqueda al SQL
    if(!empty($buscar)) { 
        $condiciones[] = "(p.descripcion LIKE ? OR m.motivo LIKE ? OR m.id = ?)"; 
        array_push($parametros, "%$buscar%", "%$buscar%", intval($buscar)); 
    }

    $sql = "SELECT m.*, p.descripcion, p.precio_costo, u.usuario FROM mermas m JOIN productos p ON m.id_producto = p.id JOIN usuarios u ON m.id_usuario = u.id WHERE " . implode(" AND ", $condiciones) . " ORDER BY m.fecha DESC";
...
<head>
    ...
    <style>
        ...
        th { background: #dc3545; color: white; padding: 10px; text-align: left; font-size: 9pt; white-space: nowrap; text-transform: uppercase; }
        td { border-bottom: 1px solid #eee; padding: 10px; font-size: 9pt; vertical-align: middle; }
        .total-row td { border-top: 2px solid #dc3545; font-weight: bold; font-size: 11pt; border-bottom: none; }
        /* Se quita el border-top aquí para evitar línea doble */
        ...
        @media (max-device-width: 768px) {
            .no-print { left: 50% !important; right: auto !important; transform: translateX(-50%) !important; bottom: 40px !important; width: 90% !important; display: flex !important; justify-content: center !important; }
            .btn-descargar { width: 100% !important; padding: 45px 20px !important; font-size: 40px !important; border-radius: 100px !important; box-shadow: 0 15px 35px rgba(0,0,0,0.4) !important; }
        }
...
<body>
    ...
    <div id="reporteContenido">
        ...
        <div class="report-page <?php echo (!$es_ultima_pagina) ? 'salto-pagina' : ''; ?>">
            <header style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #dc3545; padding-bottom: 10px; margin-bottom: 20px;">
                ...
                <div class="empresa-info" style="width: 50%; text-align: center;">
                    <h1 style="margin: 0; font-size: 18pt; color: #dc3545;"><?php echo strtoupper($negocio['nombre']); ?></h1>
            ...
            <h3 style="color: #dc3545; border-left: 5px solid #dc3545; padding-left: 10px; margin-bottom: 20px; text-transform: uppercase;">Detalle de Mermas y Bajas</h3>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%; text-align: right;">PÉRDIDA</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if(empty($chunk)): ?>
                            <tr><td colspan="6" style="text-align:center; padding: 30px;">No hubo mermas en este periodo o filtro.</td></tr>
                        <?php else: ?>
                            <?php foreach($chunk as $r): 
                                $perdida = $r['cantidad'] * $r['precio_costo'];
                                $totalPerdidaAcumulada += $perdida; 
                            ?>
                            <tr>
                                <td><?php echo date('d/m/y H:i', strtotime($r['fecha'])); ?></td>
                                <td><?php echo strtoupper($r['usuario']); ?></td>
                                <td><strong><?php echo strtoupper($r['descripcion']); ?></strong></td>
                                <td><span style="background: #eee; padding: 2px 6px; border-radius: 4px; font-size: 8pt;"><?php echo $r['motivo']; ?></span></td>
                                <td style="font-weight: bold;">-<?php echo floatval($r['cantidad']); ?></td>
                                <td style="text-align: right; font-weight: bold; color:#dc3545;">-$<?php echo number_format($perdida, 2, ',', '.'); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                        ...
                        <tr>
                            <td colspan="6" style="text-align: center; background-color: #f1f4f9; color: #666; font-size: 8.5pt; font-weight: bold; letter-spacing: 2px; padding: 15px 10px; border-top: 1px solid #ddd; border-bottom: 1px solid #ddd;">
                        ...
                        <?php if($es_ultima_pagina): ?>
                        <tr class="total-row">
                            <td colspan="5" style="text-align: right;">TOTAL PÉRDIDA VALORIZADA:</td>
                            <td style="text-align: right; color: #dc3545; white-space: nowrap !important;">-$<?php echo number_format($totalPerdidaAcumulada, 2, ',', '.'); ?></td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            ...
            <div class="footer-section">
                <div style="width: 40%; font-size: 8pt; color: #666; line-height: 1.3;">
                    <p><strong>DECLARACIÓN JURADA:</strong> Este reporte refleja fielmente las bajas de inventario registradas en el sistema.</p>
                </div>
                <div style="width: 30%; text-align: center;">
                    <img src="<?php echo "https://api.qrserver.com/v1/create-qr-code/?size=70x70&margin=0&data=" . urlencode((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]/reporte_mermas.php?publico=1&gen_by=" . $id_operador_gen); ?>" style="width: 70px; height: 70px;" alt="QR Verificación">
    ...
    function descargarPDF() {
        ...
        html2pdf().set(opt).from(element).toPdf().get('pdf').then(function (pdf) {
            ...
            for (let i = 1; i <= totalPages; i++) {
                pdf.setPage(i);
                pdf.setDrawColor(220, 53, 69); // Rojo Mermas
                pdf.setLineWidth(0.5);
                pdf.line(10, 282, 200, 282);
                pdf.setFontSize(8);
                pdf.setTextColor(100, 100, 100);
                pdf.text('Documento de Reporte de Mermas - <?php echo addslashes(strtoupper($negocio['nombre'])); ?>', 10, 287);
