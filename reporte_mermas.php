...
try {
    ...
    $sql = "SELECT m.*, p.descripcion, p.precio_costo, u.usuario FROM mermas m JOIN productos p ON m.id_producto = p.id JOIN usuarios u ON m.id_usuario = u.id WHERE DATE(m.fecha) >= ? AND DATE(m.fecha) <= ? ORDER BY m.fecha DESC";
...
<body>
    ...
    function descargarPDF() {
        ...
        const opt = { margin: 0, filename: 'Reporte_Mermas.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
