<?php
// reporte_sorteos.php - REPORTE PDF CORPORATIVO (ESTÁNDAR VANGUARD PRO)
session_start();
...
try {
    ...
    // Datos del Dueño para la Firma (Regla: Reporte = Firma Dueño)
    ...
    $sql = "SELECT s.*, u.usuario as creador FROM sorteos s LEFT JOIN usuarios u ON s.id_usuario = u.id WHERE " . implode(" AND ", $cond) . " ORDER BY s.fecha_sorteo DESC";
...
<head>
    ...
    <style>
        ...
        td { padding: 10px; font-size: 9pt; vertical-align: middle; }
        /* CSS Clave para evitar líneas dobles: Los TD no tienen borde abajo si es el total */
        tr:not(.total-row) td { border-bottom: 1px solid #eee; }
        .total-row td { border-top: 2px solid #102A57 !important; border-bottom: none !important; font-weight: bold; font-size: 11pt; padding-top: 10px; }
        ...
        /* Botón Gigante en celular aplicando !important en todo */
        ...
        .badge { padding: 4px 8px; border-radius: 4px; font-size: 8pt; font-weight: bold; text-transform: uppercase; color: white; }
...
<body>
    ...
    <div id="reporteContenido">
        ...
        $chunks = []; $temp = []; $totalRecaudacionPotencial = 0;
        ...
        <div class="report-page <?php echo (!$es_ultima_pagina) ? 'salto-pagina' : ''; ?>">
            <header style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #102A57; padding-bottom: 10px; margin-bottom: 20px;">
                ...
                <div style="text-align: right; width: 30%; font-size: 8pt;">
                    <strong>PERÍODO:</strong> <br><?php echo $rango_texto; ?><br>
                    <strong>EMISIÓN:</strong> <?php echo date('d/m/Y H:i'); ?>
                </div>
            </header>
            <h3 style="color: #102A57; border-left: 5px solid #102A57; padding-left: 10px; margin-bottom: 20px; text-transform: uppercase;">Detalle de Rifas y Campañas</h3>
            <table>
                <thead>
                    <tr><th style="width:15%">FECHA</th><th style="width:30%">CAMPAÑA</th><th style="width:10%">TICKETS</th><th style="width:15%">ESTADO</th><th style="width:15%; text-align: right;">PRECIO</th><th style="width:15%; text-align: right;">POTENCIAL</th></tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge <?php echo ($r['estado'] == 'activo') ? 'bg-success' : (($r['estado'] == 'pendiente') ? 'bg-warning' : 'bg-secondary'); ?>"><?php echo strtoupper($r['estado']); ?></span></td>
                    ...
                    <tr><td colspan="6" style="text-align: center; background-color: #f1f4f9; color: #666; font-size: 8.5pt; font-weight: bold; padding: 15px 10px;">--- CONTINÚA EN LA PÁGINA SIGUIENTE ---</td></tr>
                    <?php endif; ?>
            ...
            <?php if($es_ultima_pagina): ?>
            <div class="footer-section" style="width: 100%; display: flex; justify-content: space-between; align-items: flex-end; margin-top: 30px;">
                <div style="width: 45%; font-size: 8pt; color: #666; line-height: 1.3;">
                    <p><strong>DECLARACIÓN JURADA:</strong> Este reporte refleja fielmente las campañas de sorteos y rifas creadas en el sistema.</p>
                </div>
                <div style="width: 25%; text-align: center;">
                    <img src="<?php echo "https://api.qrserver.com/v1/create-qr-code/?size=70x70&margin=0&data=" . urlencode((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]/reporte_sorteos.php?publico=1"); ?>" style="width: 70px; height: 70px;" alt="QR">
                    <p style="font-size: 7pt; margin-top: 5px; color:#666;">Verificar online</p>
                </div>
                <div class="firma-area" style="width: 30%; text-align: center;">
                    <?php if(!empty($firmaUsuario)): ?><img src="<?php echo $firmaUsuario; ?>?v=<?php echo time(); ?>" class="firma-img" style="max-height: 80px; margin-bottom: -15px;"><?php endif; ?>
                    <div class="firma-linea" style="border-top: 1px solid #000; padding-top: 5px; font-weight: bold; font-size: 9pt;">
                        <?php echo strtoupper($firmante['nombre_completo']) . " | " . strtoupper($firmante['nombre_rol']); ?>
                    </div>
                </div>
            </div>
            <?php endif; ?>
    ...
    function descargarPDF() {
        ...
        const opt = { margin: 0, filename: 'Reporte_Sorteos.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, scrollY: 0 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
        html2pdf().set(opt).from(element).toPdf().get('pdf').then(function (pdf) {
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                pdf.text('Reporte de Sorteos - <?php echo addslashes(strtoupper($negocio['nombre'])); ?>', 10, 287);
